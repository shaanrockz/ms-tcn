    ...
    def train(self, save_dir, data_train, num_epochs, learning_rate, device):
        ...
        for epoch in range(num_epochs):
            ...
            for idx, sample_batch in enumerate(data_train):
                ...
                # break
                loss = 0
                loss += self.ce_class(pred_fg.transpose(2, 1).contiguous().view(-1, self.num_classes-1), batch_target.view(-1)-1)
                # print(p.size())
                # print(mask.size())
                ...
                loss += self.ce_bg(pred_bg.transpose(2, 1).contiguous().view(-1, 1), (batch_target.view(-1, 1)==0).float())
                #print(loss.item())
...
def main():
    ...
    trainer.train(None, tcn_dataloader, num_epochs=1, learning_rate=0.0001, device=device)
